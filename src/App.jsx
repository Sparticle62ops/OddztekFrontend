...
const TerminalLine = ({ text, type, instant }) => {
  ...
  useEffect(() => {
    ...
    if (!text) return; // Prevent crash on empty text

    let index = 0;
    const speed = type === 'error' ? 2 : 15; // Faster for errors
...
function App() {
  ...
  const [output, setOutput] = useState([
    { text: 'ODDZTEK OS v10.5 [FINAL]', type: 'system', instant: true },
    { text: 'Initializing Core Modules...', type: 'system', instant: true },
    { text: 'Type "help" to begin.', type: 'info', instant: true },
  ]);
  ...
  const initializeSystem = () => {
    ...
    newSocket.on('connect', () => {
      ...
      if (token) {
        newSocket.emit('login_token', token);
        printLine('Session Token Found. Authenticating...', 'system');
      }
    });
    ...
    newSocket.on('pong', () => printLine("Pong! Server Online.", "success"));
  ...
  const handleCommand = (cmd) => {
    ...
    // Always print user command instantly
...
[SOCIAL & MISSIONS]
  ...
  faction create [name] | faction join [name]
      ...
      case 'sandbox':
        try {
          const res = new Function('console', `return (${args.slice(1).join(' ')})`)({ log: (m) => printLine(`[JS] ${m}`) });
          if(res !== undefined) printLine(`[JS] ${res}`, 'success');
      ...
      // SOCIAL & CAMPAIGN
      case 'chat': if(args[1]) socket.emit('global_chat', args.slice(1).join(' ')); break;
      case 'mail':
